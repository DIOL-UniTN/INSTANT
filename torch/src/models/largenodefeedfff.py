...
class LargeNodeFeedFFF(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, out_features: int, depth: int):
        ...
        self.node_weights = nn.Parameter(torch.empty((self.n_nodes, self.in_features*2), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        prev_blogits = torch.zeros((x.shape), dtype=torch.float).to(x.device) # Since root node
        ...
        for d in range(self.depth.item()):
            ...
            node_in = torch.cat((x, prev_blogits[:, :original_shape[1]]), dim=1)
            ...
            platform_entropies = compute_entropy(
                boundary_effect[:, :original_shape[1]], not_boundary_effect[:, :original_shape[1]]
            ) # (batch_size, n_nodes)
            ...
            breakpoint()
            prev_blogits = node_in * current_weights

            del mixture_modifier, boundary_effect, not_boundary_effect, boundary_plane_coeff_scores, current_weights, current_biases
