...
class NodeFeedFFF(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, out_features: int, depth: int):
        ...
        if depth < 0 or self.in_features <= 0 or leaf_width <= 0 or self.out_features <= 0:
        ...
        self.node_weights = nn.Parameter(torch.empty((self.n_nodes, self.in_features+2**(depth-1)), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        ...
        self.w2s = nn.Parameter(torch.empty((self.n_leaves, leaf_width, self.out_features), dtype=torch.float).uniform_(-l2_init_factor, +l2_init_factor), requires_grad=True)
        self.b2s = nn.Parameter(torch.empty((self.n_leaves, self.out_features), dtype=torch.float).uniform_(-l2_init_factor, +l2_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        prev_boundary = torch.zeros((batch_size, 2**(self.depth - 1)), dtype=torch.float).to(x.device) # Since root node
        ...
        for d in range(self.depth.item()):
            ...
            node_in = torch.cat((x, prev_boundary), dim=1)
            boundary_plane_coeff_scores = torch.matmul(node_in, current_weights.transpose(0, 1))      # (batch_size, n_nodes)
            ...
            platform_entropies = compute_entropy(
            ...
            pad_amount = 2**(self.depth - 1) - 2**d 
            prev_boundary = torch.nn.functional.pad(boundary_effect, (0, pad_amount))

            del mixture_modifier, not_boundary_effect, boundary_plane_logits, boundary_plane_coeff_scores, current_weights, current_biases
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        prev_score = torch.zeros((batch_size, 2**(self.depth - 1)), dtype=torch.float).to(x.device) # Since root node
        for d in range(self.depth.item()):
            node_in = torch.cat((x, prev_score), dim=1)
            ...
            plane_coeff_score = torch.bmm(node_in.unsqueeze(1), plane_coeffs.unsqueeze(-1))       # (batch_size, 1, 1)
            plane_score = plane_coeff_score.squeeze(-1) + plane_offsets                     # (batch_size, 1)
            plane_choices = (plane_score.squeeze(-1) >= 0).long()                           # (batch_size,)

            platform = torch.tensor(2 ** d - 1, dtype=torch.long, device=x.device)          # (batch_size,)
            next_platform = torch.tensor(2 ** (d+1) - 1, dtype=torch.long, device=x.device) # (batch_size,)
            current_nodes = (current_nodes - platform) * 2 + plane_choices + next_platform  # (batch_size,)

            pad_amount = 2**(self.depth - 1) - plane_score.size(1)
            prev_score = torch.nn.functional.pad(plane_score, (0, pad_amount))
...
def compute_entropy(p: torch.Tensor, minus_p: torch.Tensor) -> torch.Tensor:
    EPSILON = 1e-6
