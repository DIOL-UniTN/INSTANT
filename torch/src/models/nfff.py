...
class NFFF(nn.Module): # N-ary fffs
    def __init__(self,
        ...
        self.node_boundaries = nn.Parameter(torch.empty((self.n_nodes, self.n), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for d in range(self.depth):
            ...
            current_boundaries = self.node_boundaries[platform:next_platform]   # (n_nodes, 1)
            current_biases = self.node_biases[platform:next_platform]   # (n_nodes, 1)

            boundary_plane_coeff_scores = (torch.matmul(x, current_weights.transpose(0, 1)).unsqueeze(2)*current_boundaries.unsqueeze(0))
            boundary_plane_logits = boundary_plane_coeff_scores + current_biases.transpose(0,1)# (batch_size, n_nodes)
            boundary_effects = torch.softmax(boundary_plane_logits, dim=1)
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for d in range(self.depth):
            ...
            plane_boundaries = self.node_boundaries.index_select(dim=0, 
                                                                 index=current_nodes)       # (batch_size, in_features)
