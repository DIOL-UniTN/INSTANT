...
class FFFEliminate(nn.Module):
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth.item()):
            ...
            node_x = node_x*current_weights.view(1, -1, node_x.shape[2], self.in_features)
            boundary_plane_coeff_scores = node_x.sum(dim=3).reshape(-1, n_nodes)
            node_x = self.activation(node_x.reshape(-1, 1, n_nodes, self.in_features))
        ...
        w1s = self.w1s.reshape(1, 2, -1, self.in_features, self.leaf_width)
        element_logits = (node_x.unsqueeze(-1)*w1s).sum(dim=3).reshape(-1, self.n_leaves, self.leaf_width)
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.depth.item()):
            ...
            # print(node_x.shape, 
            #       plane_coeffs.view(batch_size, -1, node_x.shape[2], self.in_features).shape
            #       )
            node_x = node_x*plane_coeffs.view(batch_size, -1, node_x.shape[2], self.in_features)
            # print(node_x.shape)
            plane_coeff_score = node_x.sum(dim=3)
            # print(node_x.shape)
        ...
        for i in range(leaves.shape[0]):
            leaf_index = leaves[i]
            logits = torch.matmul(
                node_x[i].unsqueeze(0),                  # (1, self.in_features)
