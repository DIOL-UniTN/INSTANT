...
class ConvFFF(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, 
                 out_features: int, depth: int, kernel_size: int):
        super().__init__()
        ...
        l1_init_factor = 1.0 / math.sqrt(in_features//kernel_size)
        self.node_weights = nn.Parameter(torch.empty((self.n_nodes, self.kernel_size), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth.item()):
            ...
            print(x.shape, current_weights.shape)
            boundary_plane_coeff_scores = torch.conv1d(x.unsqueeze(1), current_weights.unsqueeze(1), stride=self.kernel_size//2)
            print(boundary_plane_coeff_scores.shape)
            boundary_plane_coeff_scores = boundary_plane_coeff_scores.sum(dim=2)
            # boundary_plane_coeff_scores = torch.conv1d(x.unsqueeze(1), current_weights.unsqueeze(1), stride=self.kernel_size//2).sum(dim=2)
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.depth.item()):
            ...
            plane_coeff_score = torch.conv1d(x.unsqueeze(0), plane_coeffs.unsqueeze(1), groups=batch_size, stride=self.kernel_size//2).transpose(0, 1).sum(dim=2).unsqueeze(2)
