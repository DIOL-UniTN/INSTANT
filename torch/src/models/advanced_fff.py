...
class AFFF(nn.Module):
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        leaf_logits = torch.empty((batch_size, 4), dtype=torch.float, device=x.device)
        ...
        new_logits =  new_logits.view(*original_shape[:-1], self.out_features)
