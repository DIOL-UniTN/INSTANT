...
class RFFF(nn.Module):
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for d in range(self.depth.item()):
            ...
            res_boundary_effect = res_boundary_effect.repeat(1, 2) if d != 0 else 0
            boundary_effect = boundary_effect + res_boundary_effect
            boundary_effect /= 2 if d != 0 else 1

            res_boundary_effect = boundary_effect
