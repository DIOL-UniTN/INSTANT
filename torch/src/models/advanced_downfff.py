...
class AdvancedDownFFF(nn.Module):
    def __init__(self,
                 in_features: int,
                 leaf_width: int, out_features: int, depth: int,
                 complexity_order: int = 1, resize_rate: int = 1):
        super().__init__()
        ...
        self.complexity_order, self.resize_rate = complexity_order, resize_rate

        self.h, self.w = 28, 28
        resizers, weights, biases = [], [], []
        for i in range(self.depth):
            ratio = 2**(self.depth - i - 1) if complexity_order else 2**i
            node_h, node_w, node_in_features = (self.h*ratio*resize_rate, self.w*ratio*resize_rate, 
                                                self.h*self.w*(ratio**2)*(resize_rate**2))
            l1_init_factor = 1.0 / math.sqrt(node_in_features)
            resizers.append(Resize((node_h, node_w)))
            weights.append(nn.Parameter(torch.empty((2**i, node_in_features), 
                                                    dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True))
            biases.append(nn.Parameter(torch.empty((2**i, 1), 
                                                   dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True))

        self.resizers, self.node_weights, self.node_biases = (nn.ModuleList(resizers), 
                                                              nn.ParameterList(weights), 
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth.item()):
            ...
            boundary_plane_coeff_scores = torch.matmul(self.resize(x, current_depth), current_weights.transpose(0, 1))
    ...
    def resize(self, x, depth):
        node_in = x.reshape(-1, 1, self.h, self.w)
        node_in = self.resizers[depth](node_in)
        ratio = 2**(self.depth - depth - 1) if self.complexity_order else 2**depth
        node_in_features = self.h*self.w*(ratio**2)*(self.resize_rate**2)
        return node_in.reshape(-1, node_in_features)
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.depth.item()):
            plane_coeffs = self.node_weights[i].index_select(dim=0, index=current_nodes)       # (batch_size, in_features)
            plane_offsets = self.node_biases[i].index_select(dim=0, index=current_nodes)       # (batch_size, 1)
            plane_coeff_score = torch.bmm((self.resize(x, i)).unsqueeze(1), plane_coeffs.unsqueeze(-1))       # (batch_size, 1, 1)
