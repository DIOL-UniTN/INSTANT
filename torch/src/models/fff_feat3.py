...
class FFFFeat3(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, out_features: int, depth: int):
        ...
        self.feat_weights = nn.Parameter(torch.empty((self.n_nodes, in_features), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        self.feat_biases = nn.Parameter(torch.empty((self.n_nodes, 1), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        feat_x = x.reshape(-1, 1, x.shape[-1])
        ...
        for current_depth in range(self.depth.item()):
            ...
            feat_weights = self.feat_weights[platform:next_platform] # (n_nodes, in_features)    
            feat_biases = self.feat_biases[platform:next_platform]   # (n_nodes, 1)
            ...
            # Feat
            # print(feat_x.shape, feat_weights.unsqueeze(0).shape)
            feat_x = (feat_x * feat_weights.unsqueeze(0))
            # print(feat_x.shape, feat_biases.shape)
            # feat_x = self.activation(feat_x + feat_biases)
            feat_x = feat_x + feat_biases
            ...
            feat_x = torch.repeat_interleave(feat_x, repeats=2, dim=1)
        ...
        x = x.unsqueeze(1) + feat_x
        element_logits = (self.w1s.view(1, self.n_leaves, self.leaf_width, self.in_features)
                          * x.unsqueeze(2)).sum(dim=3)
        ...
        new_logits = torch.empty((batch_size, self.n_leaves, self.out_features), 
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.depth.item()):
            ...
            feat_coeffs = self.node_weights.index_select(dim=0, index=current_nodes).unsqueeze(1)
            feat_offsets = self.node_biases.index_select(dim=0, index=current_nodes)       # (batch_size, 1)
            plane_coeff_score = torch.bmm(x.unsqueeze(1), plane_coeffs.unsqueeze(-1))       # (batch_size, 1, 1)

            # Feat
            feat_x = feat_x * feat_coeffs
            # feat_x = self.activation(feat_x + feat_offsets.unsqueeze(2))
            feat_x = feat_x + feat_offsets.unsqueeze(2)
        ...
        x = x + feat_x.squeeze(1)
        new_logits = torch.empty((batch_size, self.out_features), 
                                 dtype=torch.float, device=x.device)
        for l in range(self.n_leaves):
            indices, = torch.where(leaves == l)
            logits = torch.matmul(x[indices], self.w1s[l]) + self.b1s[l].unsqueeze(0)                                               # (1, self.leaf_width)
