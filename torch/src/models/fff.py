...
class FFF(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, out_features: int, depth: int, 
        ...
        self.alpha = nn.Parameter(torch.tensor(sig_alpha), requires_grad=False)
    ...
    def train_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth.item()):
            ...
            boundary_effect = self.node_activation(boundary_plane_logits, self.alpha)
