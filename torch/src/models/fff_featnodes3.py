...
class FFFFeat(nn.Module):
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for d in range(self.depth.item()):
            ...
            node_out = torch.cat((node_x, node_feats), dim=2)*current_weights.unsqueeze(0)
            boundary_plane_coeff_scores, node_feats = (node_out[:, :, 0].sum(dim=2), 
                                                       node_out[:, :, 1].unsqueeze(2))
            boundary_plane_logits = boundary_plane_coeff_scores + current_biases.T
            ...
            current_mixture = current_mixture.view(batch_size, 2 * n_nodes, 
                                                   self.n_leaves // (2 * n_nodes)) # (batch_size, 2*n_nodes, self.n_leaves // (2*n_nodes))
            # print(mixture_modifier.shape, current_mixture.shape)
            ...
            node_x, node_feats = (x.repeat(1, 2**(d+1), 1, 1), 
                                  torch.repeat_interleave(node_feats, repeats=2, dim=1))
        ...
        element_logits = (self.w1s.view(1, self.n_leaves, self.leaf_width, self.in_features)*node_feats).sum(dim=3)
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for d in range(self.depth.item()):
            plane_coeffs = self.node_weights.index_select(dim=0, index=current_nodes).unsqueeze(1)
            plane_offsets = self.node_biases.index_select(dim=0, index=current_nodes)       # (batch_size, 1)

            node_out = (torch.cat((node_x, node_feats), dim=2)
                        *plane_coeffs)
            plane_coeff_score, node_feats = (node_out[:, :, 0].sum(dim=2), 
                                              node_out[:, :, 1].unsqueeze(2))
        ...
        for i in range(self.n_leaves):
            leaf_index = leaves[i]
            leaf_weights, leaf_in = (self.w1s.view(self.n_leaves, self.leaf_width, self.in_features)[leaf_index],
                                     node_feats[i, leaf_index])
            logits = (leaf_weights*leaf_in).sum(dim=1)
            logits += self.b1s[leaf_index]    # (1, self.leaf_width)
