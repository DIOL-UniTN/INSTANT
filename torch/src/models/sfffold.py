...
class SFFF(nn.Module):
    def __init__(self,
        ...
        node_dim = in_features//1
        self.stride = node_dim
        self.node_weights = nn.Parameter(torch.empty((self.n_nodes, node_dim), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        # x has shape (batch_size, in_features) original_shape = x.shape
        ...
        for current_depth in range(self.depth.item()):
            ...
            boundary_plane_coeff_scores = torch.conv1d(x.unsqueeze(1), current_weights.unsqueeze(1), stride=self.stride).sum(dim=2) # (batch_size, n_nodes)
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.depth.item()):
            ...
            plane_coeff_score = torch.conv1d(x.unsqueeze(0), plane_coeffs.unsqueeze(1), groups=batch_size, stride=self.stride).transpose(0, 1)
