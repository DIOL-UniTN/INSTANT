...
class AnyFFF(nn.Module):
    def __init__(self,
        ...
        self.onehots = torch.ones(self.n_nodes, dtype=torch.long)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        last_node_onehots = self.onehots[platform:next_platform]
        leaf_onehots = torch.repeat_interleave(last_node_onehots, repeats=2)
        leaf_indices = torch.where(leaf_onehots == 1)[0]

        mixture = current_mixture[:, leaf_indices]
        w1s, b1s = self.w1s[leaf_indices], self.b1s[leaf_indices]
        w2s, b2s = self.w2s[leaf_indices], self.b2s[leaf_indices]

        element_logits = torch.matmul(x, w1s.transpose(0, 1).flatten(1, 2))            # (batch_size, self.n_leaves * self.leaf_width)
        element_logits = element_logits.view(batch_size, self.n_leaves, self.leaf_width)    # (batch_size, self.n_leaves, self.leaf_width)
        element_logits += b1s.view(1, *self.b1s.shape)                                 # (batch_size, self.n_leaves, self.leaf_width)
        ...
        new_logits *= mixture.unsqueeze(-1)         # (batch_size, self.n_leaves, self.out_features)
