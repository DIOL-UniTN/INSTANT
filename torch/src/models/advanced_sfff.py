...
class AdvancedSFFF(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, 
                 out_features: int, depth: int,
                 complexity_order: int = 1, kernel_rate: int = 1):
        super().__init__()
        ...
        if kernel_rate > 2:
            raise ValueError("kernel rate cannot be greater than 2")

        self.n_leaves = 2 ** depth
        self.n_nodes = 2 ** depth - 1
        self.depth = nn.Parameter(torch.tensor(depth, dtype=torch.long), requires_grad=False)
        self.complexity_order, self.kernel_rate = complexity_order, kernel_rate
        ...
        for i in range(self.depth):
            ...
            kernel_dim = in_features//(2**(i+1)) if complexity_order else in_features//(2**(self.depth - i))
            print(kernel_dim)
            l1_init_factor = 1.0 / math.sqrt(kernel_dim)
        ...
        self.node_dim = in_features//node_ratio

        l1_init_factor = 1.0 / math.sqrt(self.node_dim)
        self.node_weights = nn.Parameter(torch.empty((self.n_nodes, self.node_dim), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth.item()):
            ...
            boundary_plane_coeff_scores = torch.conv1d(x.unsqueeze(1), current_weights.unsqueeze(1), stride=self.node_dim).sum(dim=2) # (batch_size, n_nodes)
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.depth.item()):
            ...
            plane_coeff_score = torch.conv1d(x.unsqueeze(0), plane_coeffs.unsqueeze(1), groups=batch_size, stride=self.node_dim).transpose(0, 1).sum(dim=2).unsqueeze(2)
