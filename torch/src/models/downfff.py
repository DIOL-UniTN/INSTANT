...
class DownFFF(nn.Module):
    def __init__(self,
                 in_features: int, resize_rate: int, 
        ...
        self.alpha = nn.Parameter(torch.ones((self.n_leaves,), dtype=torch.float), requires_grad=True)
        self.depth = nn.Parameter(torch.tensor(depth, dtype=torch.long), requires_grad=False)

        self.h, self.w = 28, 28
        self.resize_rate = resize_rate
        self.node_in_features = int(in_features//(resize_rate*resize_rate))
        self.node_h, self.node_w = ((int(self.h//resize_rate), int(self.w//resize_rate)))
        self.resizer = Resize((self.node_h, self.node_w))
        l1_init_factor = 1.0 / math.sqrt(self.node_in_features)
        self.node_weights = nn.Parameter(torch.empty((self.n_nodes, self.node_in_features), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        node_in = self.resizer(node_in)
        node_in = node_in.reshape(-1, self.node_h*self.node_w)
