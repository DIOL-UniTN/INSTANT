...
class FFF(nn.Module):
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth.item()):
            ...
            breakpoint()
            node_in = node_in.unsqueeze(1)*current_weights 
            node_in += current_biases.transpose(0, 1)
            node_in = node_in.transpose(2,3) + current_biases.transpose(0,1)
            boundary_plane_logits = node_in.sum(2)
            ...
            del mixture_modifier, boundary_effect, not_boundary_effect, boundary_plane_logits, current_weights, current_biases
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.depth.item()):
            ...
            breakpoint()
            node_in = node_in.repeat(2**i, 1, 1, 1)  
            node_in = node_in*plane_coeffs
            node_in += plane_offsets
            ...
            plane_choices = (node_in.squeeze(-1) >= 0).long()                           # (batch_size,)
