...
class Training:
    ...
    def get_config(self):
        ...
        exp_config.update(self.model.get_config()) 
        exp_config.update(self.sig_sched.get_config())
        return exp_config
    ...
    def setup(self, partial_model, optim, sched, sig_sched, loader, device):
        ...
        self.sig_sched = sig_sched(self.model.alpha)
    ...
    def log_epoch(self, train, train_inf, val, epoch, best_val, testloader, device):
        train_loss, train_acc, train_entropy, train_sample_entropy = train
        # train_inf_loss, train_inf_acc, tsdecisions, tleaftargets,  tleaf_acc = train_inf 
        # val_loss, val_acc, vsdecisions, vleaftargets, vleaf_acc = val 
        ...
        sigmoid_alpha, (lr,) = self.model.alpha.item(), self.sched.get_lr()
        ...
        logging.info("LR: {:.6f}, Sigmoid alpha {:.4}, uncertainity: {:.4f}, leaf distribution {:.4f}".format(
        ...
        if better_acc or better_loss:
            # test_loss, test_acc, _, _, _ = eval_model(self.model, testloader, device)
            # test_loss, test_acc, _, _, _, _, _, _ = eval_model(self.model, testloader, device)
            test_loss, test_acc, _, _, _, _, _ = eval_model(self.model, testloader, device)
    ...
    def run(self, cfg):
        ...
        for epoch in range(cfg.epochs):
            train = train_epoch(self.model, self.optim, cfg.loader.train, epoch, 
                                cfg.device)
            # train_inf = eval_model(self.model, cfg.loader.train_inf, cfg.device, 
            #                        leafstats)
            val = eval_model(self.model, cfg.loader.valid, cfg.device, 
                             leafstats)
            self.step_scheds()
            # pring accs
            print("Epoch: {} | train acc: {:.4f}, train inf acc: {:.4f}, val acc: {:.4f}".format(
                epoch, train[1], 0.0, val[1]))
            #best_val = self.log_epoch(train, train_inf, val, epoch, best_val, cfg.loader.test, 
            #                          cfg.device)

        # Final Test
        # test_loss, test_acc, _, _, _ = eval_model(self.model, cfg.loader.test, cfg.device, 
        #                                           leafstats)
        test_loss, test_acc, _, _, _, _, _ = eval_model(self.model, cfg.loader.test, cfg.device)
        # test_loss, test_acc, _, _, _, _, _, outs = eval_model(self.model, cfg.loader.test, cfg.device)
        # self.return_format(outs)
    ...
    def return_format(self, outs):
        input, target, out = outs
        with open(self.out_dir/"model.txt", "w") as file:
            file.write(f"input = {input}\n")
            file.write(f"target = {target}\n")
            file.write(f"node_w = {self.model.node_weights.tolist()}\n")
            file.write(f"node_b = {self.model.node_biases.tolist()}\n")
            file.write(f"leaf_w = {self.model.w1s.tolist()}\n")
            file.write(f"leaf_b = {self.model.b1s.tolist()}\n")
            file.write(f"out_w = {self.model.w2s.tolist()}\n")
            file.write(f"out_b = {self.model.b2s.tolist()}\n")
